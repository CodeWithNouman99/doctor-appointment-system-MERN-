...
const Contact = () => {
  ...
  const validate = () => {
    ...
    if (form.message.trim().length < 10) newErrors.message = 'Message should be at least 10 characters'
    return newErrors
  }
        ...
        {contactInfo.map((item) => {
          const content = (
            <>
              ...
              <p className='text-sm text-gray-500 mt-1 break-words'>{item.value}</p>
            </>
          )
          const cardClass = 'group block border border-gray-200 rounded-2xl p-6 hover:border-[#5F6FFF] hover:shadow-lg transition-all duration-300'

          return item.href ? (
            <a key={item.title} href={item.href} className={cardClass}>{content}</a>
          ) : (
            <div key={item.title} className={cardClass}>{content}</div>
          )
        })}
      </section>
          ...
          <form onSubmit={handleSubmit} noValidate className='mt-6 flex flex-col gap-5'>
            <div className='grid sm:grid-cols-2 gap-5'>
              <div>
                ...
                <input name='name' value={form.name} onChange={handleChange} placeholder='Your name' className={`mt-1.5 ${inputClass('name')}`} />
                {errors.name && <p className='text-xs text-red-500 mt-1'>{errors.name}</p>}
              </div>
              <div>
                <label className='text-sm font-medium text-gray-700'>Email Address</label>
                <input name='email' type='email' value={form.email} onChange={handleChange} placeholder='you@example.com' className={`mt-1.5 ${inputClass('email')}`} />
            ...
            <div>
              ...
              <select name='subject' value={form.subject} onChange={handleChange} className={`mt-1.5 bg-white ${inputClass('subject')}`}>
            ...
            <div>
              ...
              <textarea name='message' value={form.message} onChange={handleChange} rows={5} placeholder='How can we help you?' className={`mt-1.5 resize-none ${inputClass('message')}`} />
        ...
        <div className='lg:col-span-2 flex flex-col gap-6'>
          <div className='bg-[#5F6FFF] text-white rounded-2xl p-8'>
            ...
            <p className='text-indigo-100 text-sm mt-2 leading-6'>
              Join Prescripto to reach more patients, manage your schedule online, and grow your practice.
            </p>
            <button
              onClick={() => { setForm({ ...form, subject: 'Join as a Doctor' }); setSubmitted(false) }}
          ...
          <div className='bg-gray-50 rounded-2xl p-8'>
            <h3 className='text-xl font-semibold text-gray-900'>Careers at Prescripto</h3>
            <p className='text-gray-500 text-sm mt-2 leading-6'>
              We're building the future of healthcare booking. Learn about our team and open roles.
            </p>
      ...
      <section className='max-w-3xl mx-auto mb-20'>
        ...
        <div className='flex flex-col gap-3'>
          {faqs.map((item, index) => {
            ...
            return (
              <div key={item.q} className={`border rounded-xl transition-all ${isOpen ? 'border-[#5F6FFF]' : 'border-gray-200'}`}>
                <button
                  onClick={() => setOpenFaq(isOpen ? null : index)}
                  className='w-full flex justify-between items-center gap-4 text-left px-6 py-4 font-medium text-gray-900'
                >
                  {item.q}
                  <span className={`text-[#5F6FFF] text-xl transition-transform duration-300 ${isOpen ? 'rotate-45' : ''}`}>+</span>
                </button>
                {isOpen && <p className='px-6 pb-5 text-sm text-gray-500 leading-6'>{item.a}</p>}
              </div>
            )
          })}
        </div>
